...
RSpec.describe Sync::CreditCard::Stripe::Outbound::DeleteJob, type: :job do
  let(:resource) { create(:payment_method) }

  let(:job) { Sync::CreditCard::Stripe::Outbound::DeleteJob }
  ...
  describe "#policy" do
    it "returns a Sync::CreditCard::Stripe::Outbound::DeletePolicy" do
      expect_any_instance_of(job).to(receive(:resource).at_least(:once).and_return(resource))
      expect(job.new(resource).policy).to be_a(Sync::CreditCard::Stripe::Outbound::DeletePolicy)
    end
  end
end
