...
RSpec.describe Sync::Webhook::ArrivyPolicy, type: :policy do
  let(:payload) {
    [
      {
        "eventId"=>843593753,
  ...
  describe_rule :enabled? do
    it "returns true if handler enabled" do
      allow(ENV).to receive(:[]).with('ARRIVY_WEBHOOK_DISABLED').and_return(nil)
      expect(policy.enabled?).to be_truthy
    end

    it "returns false if handler disabled" do
      allow(ENV).to receive(:[]).with('ARRIVY_WEBHOOK_DISABLED').and_return("true")
