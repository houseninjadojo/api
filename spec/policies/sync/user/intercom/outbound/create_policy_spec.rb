...
RSpec.describe Sync::User::Intercom::Outbound::CreatePolicy, type: :policy do
  ...
  describe_rule :has_external_id? do
    it "returns true if user has intercom_id" do
      user.intercom_id = "1234"
      expect(policy.has_external_id?).to be_truthy
    end

    it "returns false if user does not intercom_id" do
